...
double run_away_velocity(double cur_x, double cur_y, double nearest_point_x, double nearest_point_y) {
	...
	double distance = sqrt(pow(nearest_point_x - cur_x, 2) + pow(nearest_point_y - cur_y, 2));
	run_away_velocity = repulsion / (distance*distance);
	return run_away_velocity;
}

double* setv(int curx, int cury, int start_x, int start_y, int goalx, int goaly, double oritation, int flag)
{
	double *vel = new double[2];
	int dx = goalx - curx;
	int dy = goaly - cury;
		...
		while (i < pointnumber_smooth - 1)
		{
			...
			if (i < pointnumber_smooth - 2) {
				...
				if (temp_dis <= 25) {
					...
					double away_velocity;
					run_velocity = run_away_velocity(x_1, y_1, get_nearest_point[0], get_nearest_point[1]);
					double *vel_revise;
					vel_revise[0] = vel[0] + run_velocity * cos(run_away_orientation_angle);
					vel_revise[1] = vel[1] + run_velocity * sin(run_away_orientation_angle);
					//	double *oba;
				...
				udp_send.send(test_car_number, vel[0], vel[1], 0, false, 0, 2);
			}

			else if (i == pointnumber_smooth - 2) {
